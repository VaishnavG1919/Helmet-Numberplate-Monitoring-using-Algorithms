...
def is_valid_plate(plate_text):
    pattern = r'^[A-Z0-9]{4,10}$'  # 4-10 alphanumeric characters
...
plate_model = YOLO(r"C:\Users\saiva\Downloads\YOLO\license_plate_detector.pt")  # your plate model
helmet_model = YOLO(r"C:\Users\saiva\Downloads\YOLO\helmet_detector.pt")        # your helmet model
reader = easyocr.Reader(['en'], gpu=True)

# Create output folders
os.makedirs("output", exist_ok=True)
os.makedirs("output/violations", exist_ok=True)
os.makedirs("output/violations/no_helmet", exist_ok=True)
os.makedirs("output/violations/no_plate", exist_ok=True)
...
while cap.isOpened():
    ...
    if len(plate_results.boxes) == 0:
        ...
        try:
            cv2.imwrite(f"output/violations/no_plate/frame{frame_count}.png", frame)
    ...
    for i, box in enumerate(plate_results.boxes):
        ...
        if text_results:
            ...
            if confidence > OCR_CONFIDENCE_THRESHOLD and is_valid_plate(plate_text):
                ...
                ground_truth = "ABC123"  # placeholder
                ...
                if confidence < OCR_CONFIDENCE_THRESHOLD:
                    print(f"Tampering suspected at frame {frame_count}: Low OCR confidence")
                ...
                if not is_valid_plate(plate_text):
                    print(f"Violation at frame {frame_count}: Invalid plate '{plate_text}'")
        ...
        # White box for text/plate above vehicle
        box_height = 100
    ...
    if len(helmet_results.boxes) == 0:
        print(f"Violation at frame {frame_count}: No helmet detected")
        try:
            cv2.imwrite(f"output/violations/no_helmet/frame{frame_count}.png", frame)
        except PermissionError as e:
            print(f"PermissionError: {e}")
        violation_writer.writerow([frame_count, "No Helmet Detected", "No helmets found"])

    for i, box in enumerate(helmet_results.boxes):
        x1, y1, x2, y2 = map(int, box.xyxy[0])
        conf = box.conf[0]
